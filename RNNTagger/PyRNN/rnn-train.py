...
def training(args):
   ...
   if args.crf_epochs > 0:  # use a CRF?
      crf_model = CRFTagger(data.num_char_types, data.num_tag_types,
      ...
      model = RNNTagger(data.num_char_types, data.num_tag_types,
                        args.char_embedding_size, data.word_emb_size,
                        args.char_recurrent_size, args.word_recurrent_size, 
                        args.char_rnn_depth, args.word_rnn_depth, args.dropout_rate)
      if args.gpu >= 0:
         model = model.cuda()
