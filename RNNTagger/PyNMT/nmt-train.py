...
def training(args):
   ...
   model = NMTDecoder(data.src_vocab_size, data.tgt_vocab_size, args.word_emb_size,
                      args.enc_rnn_size, args.dec_rnn_size, args.enc_depth, args.dec_depth, 
                      args.dropout_rate, args.emb_dropout_rate, args.use_lstm, args.tie_embeddings)
   ...
   for batch_no, batch in enumerate(data.train_batches(), 1):
      ...
      if batch_no % args.eval_interval == 0:
         ...
         tgtIDs = model.translate(src_wordIDs, src_len)
         ...
         # if best_dev_loss > loss_sum:
         #    best_dev_loss = loss_sum
         #    print("storing parameters", file=sys.stderr)
         #    data.store_parameters( args.path_param+".io" )
         #    if model.on_gpu():
         #       model = model.cpu()
         #       torch.save( model, args.path_param+".nmt" )
         #       model = model.cuda()
         #    else:
         #       torch.save( model, args.path_param+".nmt" )
         # loss_sum = 0.0
...
if __name__ == "__main__":
   ...
   parser.add_argument("--use_lstm", action="store_true", default=False,
                       help="use LSTMs instead of GRUs")
